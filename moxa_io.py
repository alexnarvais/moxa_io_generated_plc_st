            ...
            for slot_name in rack.keys():
                ...
                match slot_type:
                    case "e1210":
                        ...
                        output_text_list.extend(e1210.rack_slot_e1210_list)
                    case "e1212":
                        output_text_list.append('/////////////////////////////////////////////////////////////////////')
                        e1212 = MoxaE1212(int(rack_name), int(slot_name), slot_type, slot)
                        output_text_list.extend(e1212.rack_slot_e1212_list)
                    case "e1240":
                        output_text_list.append('/////////////////////////////////////////////////////////////////////')
                        e1240 = MoxaE1240(int(rack_name), int(slot_name), slot_type, slot)
                        output_text_list.extend(e1240.rack_slot_e1240_list)
                    case "e1241":
                        output_text_list.append('/////////////////////////////////////////////////////////////////////')
                        e1241 = MoxaE1241(int(rack_name), int(slot_name), slot_type, slot)
                        output_text_list.extend(e1241.rack_slot_e1241_list)
                    case "e1260":
                        output_text_list.append('/////////////////////////////////////////////////////////////////////')
                        e1260 = MoxaE1260(int(rack_name), int(slot_name), slot_type, slot)
                        output_text_list.extend(e1260.rack_slot_e1260_list)
                    case _:
                        print("No match?")
...
class MoxaE1200(ABC):
    def __init__(self, rack_num, slot_num, slot_type):
        ...
        self._rs = f'r{self._rack_num:02}s{self._slot_num:02}'
        self._comm_config = [f'// Rack {self._rack_num} Slot {self._slot_num}',
                     '// This code was generated by python. Please make corrections in the csv file, rerun the code and re-paste',
                     '// Get the module status',
                     f'GSV(Module, {self._rs}_moxa_{self._slot_type}, EntryStatus, r{self._rack_num:02}s{self._slot_num:02}_module_status);',
                     '// Get 4 bits from 12 - 15 and put into separate DINT',
                     f'{self._rs}_btdt_1.Source := {self._rs}_module_status;',
                     f'{self._rs}_btdt_1.SourceBit := 12;',
                     f'{self._rs}_btdt_1.Length := 4;',
                     f'{self._rs}_btdt_1.DestBit := 0;',
                     f'{self._rs}_btdt_1.Target := zero;',
                     f'BTDT({self._rs}_btdt_1);',
                     f'{self._rs}_module_status_easy := {self._rs}_btdt_1.Dest;',
                     '',
                     '// If this is the first time the DINT is equal to something other than 4, see why',
                     f'{self._rs}_osri_1.InputBit := ({self._rs}_module_status_easy <> 4);',
                     f'OSRI({self._rs}_osri_1);',
                     f'if {self._rs}_osri_1.OutputBit then',
                     f'   GSV(Module, {self._rs}_moxa_{self._slot_type}, FaultCode, {self._rs}_fault_code);',
                     f'   GSV(Module, {self._rs}_moxa_{self._slot_type}, FaultInfo, {self._rs}_fault_info);',
                     'end_if;',
                     '',
                     '// The connection is good, if the module states is equal to 4',
                     f'{self._rs}_connection_error := ({self._rs}_module_status_easy <> 4);',
                     '',
                     '// Set the av program input for the fault code',
                     f'AOI_AV(aoi_{self._rs}_comm_fault_code, av_{self._rs}_comm_fault_code);',
                     f'av_{self._rs}_comm_fault_code.Program_Input := {self._rs}_fault_code;',
                     '',
                     '// Set the av program input for the fault info',
                     f'AOI_AV(aoi_{self._rs}_comm_fault_info, av_{self._rs}_comm_fault_info);',
                     f'av_{self._rs}_comm_fault_info.Program_Input := {self._rs}_fault_info;',
                     '',
                     '// Set the comm status dv',
                     f'AOI_DV(aoi_{self._rs}_comm_status, dv_{self._rs}_comm_status);',
                     f'dv_{self._rs}_comm_status.Program_Input := NOT {self._rs}_connection_error AND NOT {self._rs}_moxa_{self._slot_type}:I.ConnectionFaulted;',
                     '']

    # These properites will be used in the child classes to access the private attributes defined here in the parent class.
    @property
    def slot_type(self):
    ...
    def comm_config(self, comm):
        self._comm_config = comm

    def clear_unused_bits_analog(self):
        rsl = ['// Clear the unused bits in the unreliable DINT',
               f'{self._rs}_btdt_2.Source := zero;',
               f'{self._rs}_btdt_2.SourceBit := 0;',
               f'{self._rs}_btdt_2.Length := 24;',
               f'{self._rs}_btdt_2.DestBit := 8;',
               f'{self._rs}_btdt_2.Target := {self._rs}_unreliable;',
               f'BTDT({self._rs}_btdt_2);',
               f'{self._rs}_unreliable := {self._rs}_btdt_2.Dest;',
               '',
               '// Clear the unused bits in the manual DINT', f'{self._rs}_btdt_3.Source := zero;',
               f'{self._rs}_btdt_3.SourceBit := 0;', f'{self._rs}_btdt_3.Length := 24;',
               f'{self._rs}_btdt_3.DestBit := 8;',
               f'{self._rs}_btdt_3.Target := {self._rs}_manual;',
               f'BTDT({self._rs}_btdt_3);',
               f'{self._rs}_manual := {self._rs}_btdt_3.Dest;',
               '']
        return rsl
        ...
        for channel_number in channel_list:
            ...
            if not tag_name:
                ...
                if channel['channel_type'] == 'di':
                    ...
                    self._rsl_e1212.append(f'AOI_DI(aoi_{self.rack_slot}c{cn:02}, {tag_name});')
                    ...
                    self._rsl_e1212.append(
                        f'{tag_name}.Program_Input := {self.rack_slot}_moxa_{self.slot_type}:I.Data[{word}].{bit};')
                    self._rsl_e1212.append('// Set the channel in manual bit in the manual DINT')
                    self._rsl_e1212.append(f'{self.rack_slot}_manual.{cn} := {tag_name}.Manual;')
                else:
                    self._rsl_e1212.append(f'// Channel {cn} - output - {tag_name}')
                    self._rsl_e1212.append('// Process the AOI')
                    self._rsl_e1212.append(
                        f'AOI_DO(aoi_{self.rack_slot}c{cn:02}, {self.rack_slot}c{cn:02}, {tag_name});')
                    self._rsl_e1212.append('// set the output value from the intermediate BOOL')
                    bit = cn - 8  # only channels 8-15 can be outputs
                    self._rsl_e1212.append(
                        f'{self.rack_slot}_moxa_{self.slot_type}:O.Data[0].{bit} := {self.rack_slot}c{cn:02};')
            self._rsl_e1212.append('')

    @property
    def rack_slot_e1212_list(self):
        return self._rsl_e1212

    @rack_slot_e1212_list.setter
    def rack_slot_e1212_list(self, rsl_e1212):
...
class MoxaE1210(MoxaE1200):
    def __init__(self, rack_num, slot_num, slot_type, slot):
        ...
        self._slot = slot
        self._e1210_config = [f'// Moxa Digital Input E1210']
        self._e1210_config.extend(self.comm_config)
        # channels
        channels = self._slot['channels']
        channel_list = [int(x) for x in list(channels.keys())]
        channel_list.sort()
        ...
        for channel_number in channel_list:
            ...
            if not tag_name:
                ...
                self._rsl_e1210.append(f'AOI_DI(aoi_{self.rack_slot}c{cn:02}, {tag_name});')
                ...
                self._rsl_e1210.append(f'{self.rack_slot}_manual.{cn} := {tag_name}.Manual;')
            self._rsl_e1210.append('')

    @property
    def e1210_config(self):
        return self._e1210_config

    @e1210_config.setter
    def e1210_config(self, e1210):
        self._e1210_config = e1210
    ...
    def __init__(self, rack_num, slot_num, slot_type, slot):
        ...
        for channel_number in channel_list:
            ...
            if not tag_name:
                ...
                self._rsl_e1240.append(f'{self.rack_slot}_unreliable.{cn} := 0;')
                self._rsl_e1240.append('// Ensure manual bit is not set')
                self._rsl_e1240.append(f'{self.rack_slot}_manual.{cn} := 0;')
            else:
                ...
                self._rsl_e1240.append(
                    f'{self.rack_slot}_btdt_c{cn:02}.Source := {self.rack_slot}_moxa_{self.slot_type}:I.Data[{channel_number}];')
                self._rsl_e1240.append(f'{self.rack_slot}_btdt_c{cn:02}.SourceBit := 0;')
                self._rsl_e1240.append(f'{self.rack_slot}_btdt_c{cn:02}.Length := 16; ')
                self._rsl_e1240.append(f'{self.rack_slot}_btdt_c{cn:02}.DestBit := 0;')
                self._rsl_e1240.append(f'{self.rack_slot}_btdt_c{cn:02}.Target := zero;')
                self._rsl_e1240.append(f'BTDT({self.rack_slot}_btdt_c{cn:02});')
                self._rsl_e1240.append(f'{self.rack_slot}c{cn:02} := {self.rack_slot}_btdt_c{cn:02}.Dest;')
                self._rsl_e1240.append('// Process the AOI')
                self._rsl_e1240.append(f'AOI_AI(aoi_{self.rack_slot}c{cn:02}, {self.rack_slot}c{cn:02}, {tag_name});')
                ...
                self._rsl_e1240.append(f'{self.rack_slot}_unreliable.{cn} := {tag_name}.Fault.Fault;')
                self._rsl_e1240.append('// Set the channel in manual bit in the manual DINT')
                self._rsl_e1240.append(f'{self.rack_slot}_manual.{cn} := {tag_name}.Manual;')
            self._rsl_e1240.append("")

    @property
    def rack_slot_e1240_list(self):
        return self._rsl_e1240

    @rack_slot_e1240_list.setter
    def rack_slot_e1240_list(self, rsl_e1240):
    ...
    def __init__(self, rack_num, slot_num, slot_type, slot):
        ...
        for channel_number in channel_list:
            ...
            if not tag_name:
                ...
                self._rsl_e1241.append(f'AOI_AO(aoi_{self.rack_slot}c{cn:02}, {self.rack_slot}c{cn:02}, {tag_name});')
                ...
                self._rsl_e1241.append(f'{self.rack_slot}_btdt_c{cn:02}.Source := {self.rack_slot}c{cn:02};')
                self._rsl_e1241.append(f'{self.rack_slot}_btdt_c00.SourceBit := 0;')
                self._rsl_e1241.append(f'{self.rack_slot}_btdt_c{cn:02}.Length := 16;')
                self._rsl_e1241.append(f'{self.rack_slot}_btdt_c{cn:02}.DestBit := 0;')
                self._rsl_e1241.append(f'{self.rack_slot}_btdt_c{cn:02}.Target := zero;')
                self._rsl_e1241.append(f'BTDT({self.rack_slot}_btdt_c{cn:02});')
                self._rsl_e1241.append(
                    f'{self.rack_slot}_moxa_{self.slot_type}:O.Data[{cn}] := {self.rack_slot}_btdt_c{cn:02}.Dest;')
            self._rsl_e1241.append(" ")

    @property
    def rack_slot_e1241_list(self):
        return self._rsl_e1241

    @rack_slot_e1241_list.setter
    def rack_slot_e1241_list(self, rsl_e1241):
    ...
    def __init__(self, rack_num, slot_num, slot_type, slot):
        ...
        for channel_number in channel_list:
            ...
            if not tag_name:
                ...
                self._rsl_e1260.append(f'{self.rack_slot}_unreliable.{cn} := 0;')
                self._rsl_e1260.append('// Ensure manual bit is not set')
                self._rsl_e1260.append(f'{self.rack_slot}_manual.{cn} := 0;')
            else:
                self._rsl_e1260.append(f'// Channel {cn} - {tag_name}')
                self._rsl_e1260.append(
                    f'{self.rack_slot}c{cn:02} := {self.rack_slot}_moxa_{self.slot_type}:I.Data[{channel_number}];')
                ...
                # self._rsl_e1260.append(f'AOI_JCI_Nickel(aoi_{self.rack_slot}c{cn:02}_jci, {self.rack_slot}c{cn:02}r, {self.rack_slot}c{cn:02}f, JCI_Nickel_RTD);')
                self._rsl_e1260.append('// Process the AOI')
                # if using jci nickel use the next line. if not use the line after
                # self._rsl_e1260.append(f'AOI_AI(aoi_{self.rack_slot}c{cn:02}, {self.rack_slot}c{cn:02}f, {tag_name});')
                self._rsl_e1260.append(f'AOI_AI(aoi_{self.rack_slot}c{cn:02}, {self.rack_slot}c{cn:02}, {tag_name});')
                ...
                self._rsl_e1260.append(f'{self.rack_slot}_unreliable.{cn} := {tag_name}.Fault.Fault;')
                self._rsl_e1260.append('// Set the channel in manual bit in the manual DINT')
                self._rsl_e1260.append(f'{self.rack_slot}_manual.{cn} := {tag_name}.Manual;')
            self._rsl_e1260.append("")

    @property
    def rack_slot_e1260_list(self):
        return self._rsl_e1260

    @rack_slot_e1260_list.setter
    def rack_slot_e1260_list(self, rsl_e1260):
